...
public class ParticleManager {
	...
	public ParticleManager(double windVelocity, double launchAngle) throws EnvironmentException, EmitterException {
		...
		maxExitVelocity = 22;
		emit = new StarEmitter(Math.sin(la), Math.cos(la), 0, 0, maxExitVelocity, launchAngle, 2);
		lastTime = 0;
	} // end constructor
	...
	private void update(double time) {
		...
		// collection from becoming empty by adding five delay charge sparks, and then start the launch
		// of another star.
		if (fireworks.size() == 0) {
	...
	/**
	 * An accessor for the collection of particles. This method does not, but should clone the collection.
